...
def parseUrlContent(url, data, regexp, compFlags = None, posIni = 0, posFin = 0):
    ...
    for key in ['url', 'videoUrl', 'iconImage', 'thumbnailImage']:
        if regexp.find('?P<' + key +'>') != -1:
            for elem in matchs:
                elem[key] = urlparse.urljoin(url, elem[key])
    if regexp.find('?P<label>') != -1 and matchs:
        srchKeys = [key for key in matchs[0].keys()  if key.startswith('label') and key not in ['label', 'label2']]
        srchKeys.sort()
        htmlUnescape = HTMLParser.HTMLParser().unescape
        for k in range(len(matchs)):
            if srchKeys:
                lista = [matchs[k].pop(key) for key in srchKeys]
                matchs[k]['label'] += ' ' + ' '.join([label for label in lista if label])
            matchs[k]['label'] = htmlUnescape(matchs[k]['label'])
    return matchs
