...
class KodiPluginImportFinder(object):
    def __init__(self, installedPluginDir, testPluginGen = None):
        self.pluginRoot = installedPluginDir
        self.testPluginGen = testPluginGen
        self.pathStack = []
        self._loaders = {}
    ...
    def getRootPath(self, path, baseName):
        self.setSearchPath()
        testPaths = []
        dPath = path[0] if path else self.pluginRoot 
    ...
    def find_module(self, fullname, path = None):
        ...
        if os.path.exists(fullpath):
            pluginRootTest = basePath == self.pluginRoot
            loader = PluginMetaImportLoader(fullpath, isPackage = True, isPluginRoot = pluginRootTest)
            try:
                self.pathStack.append(fullpath)
                mod = loader.load_module(fullname)
                self._loaders[fullpath] = PluginMetaImportLoader(fullpath)
            except ImportError:
                loader = None
            finally:
                self.pathStack.pop()
        ...
        if os.path.exists(fullname):
            if fullname not in self._loaders:
                self._loaders[fullname] = PluginMetaImportLoader(basePath)
...
class PluginMetaImportLoader(object):
    def __init__(self, addonPath, isPackage = False, isPluginRoot = False):
        self.addonPath = addonPath
        self._isPackage = isPackage
        self.isPluginRoot = isPluginRoot
        self._source_cache = {}
        self.co_flags = 0
    ...
    def get_source(self, fullname):
        ...
        if not os.path.exists(filename): return ""  # Asumo que se trata del directorio raiz del plugin
