...
class Dialog(object):
    ...
    def numeric(self, type, heading, default=None):
        ...
        if OUTPUT_TYPE == 'console':
            prompt = heading + (('[' + default + ']') if default else '')
            nLetters = len(prompt)
            print(nLetters*'=')
            print (nLetters - len(heading))/2*' ', heading
            print(nLetters*'-')
            answ = raw_input("Input format " + formatStr[type] + ":") or default
            return answ
        elif OUTPUT_TYPE == 'tkinter':
            import tkSimpleDialog
            return tkSimpleDialog.askstring('Numeric Dialog', "Input format " + formatStr[type] + ":")
    ...
    def notification(self, heading, message, icon=None, time=None, sound=None):
        ...
        """
        import time as xbmcTime
        prompt = [message]
        nLetters = max(map(len,[heading] + prompt))
        if OUTPUT_TYPE == 'console':
            print(nLetters*'=')
            print (nLetters - len(heading))/2*' ', heading
            print(nLetters*'-')
            for elem in prompt:
                if elem: print (elem)
            print(nLetters*'=')
            if not time: time = 5000
            xbmcTime.sleep(time/1000)
        elif OUTPUT_TYPE == 'tkinter':
    ...
    def yesno(self, heading, line1, line2=None, line3=None, nolabel=None, yeslabel=None):
        ...
        if OUTPUT_TYPE == 'console':
            ...
            print(nLetters*'=')
            if not yeslabel: yeslabel = 'Y'
            if not nolabel: nolabel = 'N'
            
            answ = raw_input(yeslabel + '/' + nolabel + '?')
            return answ.lower() == yeslabel.lower()
        elif OUTPUT_TYPE == 'tkinter':
            message = '\n'.join(prompt)
            ans = tkMessageBox.askyesno(heading, message)
            return ans[0]
    ...
    def ok(self, heading, line1, line2=None, line3=None):
        ...
        if OUTPUT_TYPE == 'console':
            ...
            print(nLetters*'=')
            answ = raw_input('Presione ENTER para continuar: ')
        elif OUTPUT_TYPE == 'tkinter':
            message = '\n'.join(prompt) 
    ...
    def select(self, heading, mlist, autoclose=0):
        ...
        """
        if OUTPUT_TYPE == 'console':
            nLetters = max(map(len,[heading] + mlist))
            print(nLetters*'=')
            print ((nLetters - len(heading))/2)*' ', heading
            print(nLetters*'-')
            for k, elem in enumerate(mlist,1):
                print k, elem
            print(nLetters*'=')
            return input('Entre item a seleccionar ') - 1
        elif OUTPUT_TYPE == 'tkinter':
            import Tkinter as tk
            class kodiSelectClass(tk.Toplevel):
                def __init__(self, memList, title = None, master = None, dheight = 600, dwidth = 500):
                    if master:
            ...
            root = tk.Tk()
            root.withdraw()
            lOptions = kodiSelectClass(mlist, title = heading)
...
class DialogProgress(object):
    ...
    def create(self, heading, line1=None, line2=None, line3=None):
        ...
        lista = [elem for elem in [line1, line2, line3] if elem]
        nLetters = max(map(len,[heading] + lista))
        print(nLetters*'=')
        print ((nLetters - len(heading))/2)*' ', heading
        print(nLetters*'-')
        for elem in lista:
    ...
    def update(self, percent, line1=None, line2=None, line3=None):
        ...
        """
        lista = [elem for elem in [line1, line2, line3] if elem]
        nLetters = max(map(len,[self.heading] + lista))
        print(nLetters*'=')
        print ((nLetters - len(self.heading))/2)*' ', self.heading
        print(nLetters*'-')
        for elem in lista:
            print elem
        print(nLetters*'=')
