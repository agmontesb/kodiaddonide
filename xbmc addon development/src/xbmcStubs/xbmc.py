...
def log(msg, level = LOGNOTICE):
    ...
    *Note, You can use the above as keywords for arguments and skip certain optional arguments.
    ...
    if xbmcLoglevel == -1: return
    if xbmcLoglevel == -0 and level not in [LOGNOTICE, LOGERROR, LOGSEVERE, LOGFATAL]: return
...
def translatePath(path):
    ...
    *Note, Only useful if you are coding for both Linux and Windows.
    ...
    if sys.platform[:3] == 'win':
        ...
        specialProtocol['special://Kodi'] = os.path.join(os.path.expandvars("$PROGRAMFILES"), "Kodi")    
        # 'C:\\Users\\$USERNAME\\AppData\\Roaming\\Kodi'
        specialProtocol['special://home'] = os.path.join(os.path.expandvars("$APPDATA"), "Kodi")
        
    while 1:
        match = re.match('(special://[^\\\\/]+)(.*)', path)
        if not match: break
        preffix, suffix = match.groups()
        try:
            specialPath = specialProtocol[preffix]
        except:
            raise LookupError(preffix +' no se encuentra definido en el Kodi')
        else:
            path = specialPath + suffix
