...
import tkFont
# import xbmc, xbmcgui, xbmcaddon, xbmcplugin, xbmc
import traceback
...
# from xbmcProgram import setPythonPath
xbmc, xbmcgui, xbmcaddon, xbmcplugin = 1, 2, 3, 4
...
class KodiFrontEnd(tk.Frame):
    def __init__(self, master = None, threadSource = None):
        tk.Frame.__init__(self, master)
        self.importXbmcStubs()
        ...
        self.testRootDir = r'H:\Kodi\portable_data\addons'
    ...
    def initGlobals(self):
        ...
        theGlobals['xbmc'].translatePath = self.translatePath(xbmc.translatePath, self.testAddon, self.testRootDir)
    ...
    def importXbmcStubs(self):
        import importlib
        for modName in ['xbmc', 'xbmcgui', 'xbmcaddon', 'xbmcplugin', 'xbmc']:
            globals()[modName] = importlib.import_module('xbmcStubs.' + modName)
            sys.modules[modName] = sys.modules.pop('xbmcStubs.' + modName)
...
#         self.listbox.focus_set()
    ...
    def runAddon(self, url):
        ...
        if urlScheme.scheme == 'plugin':             # Plugin diferente
            ...
            if self.addonID != actualID:
                ...
                self.theGlobals['sys'].path = self.setPythonPath(actualID) + self.ORIGINAL_PYTHONPATH
    ...
    def makeWidgets(self):
        ...
        allContainer = collapsingFrame.collapsingFrame(self,tk.BOTTOM)
        allContainer.pack(side = tk.TOP, fill = tk.BOTH, expand = tk.YES)
        bottomFrame = tk.Frame(allContainer.bottonBand, height = 200)
        ...
        self.stEd = stEd
        topFrame = tk.Frame(allContainer.topBand)
        ...
        self.sbar = sbar
        infoPane = collapsingFrame.collapsingFrame(topFrame, tk.BOTTOM)        
        infoPane.pack(side = tk.LEFT, fill = tk.BOTH, expand = tk.YES)
        infoPaneUp = tk.Frame(infoPane.topBand)
        ...
        self.thumbnail.pack()
        labelPane = tk.Label(infoPane.bottonBand, textvariable = self.message, relief=tk.SUNKEN)
